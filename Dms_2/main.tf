...
resource "aws_dms_endpoint" "source_endpoint" {
  ...
  password      = "Admin@123"
  port          = 3306
  server_name   = "ec2-3-216-94-244.compute-1.amazonaws.com" #relace it is manual process you have to go to your database instance(database-mysql)-->copy Public IPv4 DNS--->paste here
...
resource "aws_dms_endpoint" "target_endpoint" {
  ...
  port          = 3306
  server_name   = "my-rds-db.cwt8642ugxeh.us-east-1.rds.amazonaws.com" #databade end point
  depends_on    = [aws_dms_replication_instance.dms_replication_instance]
